...
## 🔖 איפה עצרנו (עודכן 2026-09-22, לילה) — קראו קודם
...
- **סטטוס:** הקוד כתוב, נפרס בשרת הפיתוח וב-`main`, **אבל כבוי** (`CUSTOMER_LOGIN_MODE` לא מוגדר ← הכניסה הישנה בסיסמה ממשיכה לעבוד ללא שינוי). אף SMS אמיתי לא נשלח מעולם.
- **הצעד הבא (מחר):** (1) חשבון ספק SMS (נכתב adapter ל-019sms, **לא נבדק מול חשבון**, פורמט ה-auth ב-`sms019.ts` ניחוש — לאמת); (2) בדיקה בדפדפן במצב `SMS_PROVIDER=mock` (הזרימה האינטראקטיבית טרם נבדקה, רק רינדור העמוד); (3) בדיקת dry-run מול endpoint הבדיקות של 019; (4) הדלקה. פירוט מדויק בסעיף 5 במסמך.
