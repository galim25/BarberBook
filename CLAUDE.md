...
- **Push גם ללקוחות + התראות מנהל על שינויים (2026-09-21, נכתב בקוד — טרם נבדק בדפדפן/מכשיר אמיתי, וטרם נפרס):** אותו מנגנון Web Push וטבלת `PushSubscription` (בלי migration) — `subscribeToPushAction` פתוחה עכשיו לכל משתמש מחובר (לא רק אדמין), עם אימות `endpoint` מול allowlist של שירותי push אמיתיים (`lib/pushEndpoint.ts`, מונע SSRF כי השרת שולח POST לכתובת) ותקרת 10 מכשירים למשתמש. `<PushNotificationToggle audience="customer"/>` ב-`/account`. `push.ts`: `sendPushToUser`/`sendPushToCustomers` בנוסף ל-`sendPushToAdmins`, וכולן לא זורקות לעולם (כשל push לא מפיל קביעה/ביטול). **ללקוח:** `sendCustomerNotification` שולחת עכשיו גם push — מכסה שינוי/ביטול תור ע"י הספר, החלטות על בקשות, ו-3 טריגרי רשימת ההמתנה (תור שהתפנה / שעות שהורחבו / יום חדש); `adminRescheduleAppointmentAction` עברה מ-SMS+`Notification` ידניים ל-`sendCustomerNotification`. הודעה חדשה מהספר (`createAnnouncementAction`) → push לכל מנויי הלקוחות בלבד (בלי שורת `Notification`). **לספר:** `notifyAdmin.ts` עבר ל-helper אחד (`notifyAdmins`) ונוספו `notifyAdminsOfCustomerReschedule` (לקוח העביר תור) ו-`notifyAdminsOfCustomerCancellation` (לקוח ביטל מיידית כשהמדיניות כבויה — קודם לא היה שום דבר); שתיהן `type: appointment_changed`. **באג שתוקן:** `adminNotifications.ts` סינן רק `appointment_booked`, ולכן `booking_request_pending`/`cancellation_request_pending` לא הופיעו בפיד/ב-badge — עכשיו `ADMIN_NOTIFICATION_TYPES`. **החלטת מוצר פתוחה:** התראות "תור התפנה/יום חדש" נשלחות רק לרשימת ההמתנה (כמו קודם) ולא לכל לקוח עם push. **פערים ידועים:** תזכורות ה-worker (`apps/worker`) עדיין בלי push (חבילה נפרדת בלי גישה ל-`apps/web/src/lib/push.ts`); העברת תור ע"י לקוח/ספר לא מודיעה לרשימת ההמתנה על השעה הישנה שהתפנתה; SMS עדיין `NoopSmsProvider` (איפוס סיסמה לא שולח קוד).
- **Dependabot: 2 alerts חדשים בחומרה גבוהה תוקנו (2026-09-21)** — אותה שיטה כמו למטה, bump ל-`overrides` הקיימים ב-`pnpm-workspace.yaml`: `sharp` `^0.35.3`→`^0.35.4` (GHSA-rgj7-g3m4-5g8c/CVE-2026-84383, חולשות RCE ב-libheif — **תלות runtime** דרך `next`'s image optimizer, לא build-time בלבד כמו רוב השאר ברשימה כאן) ו-`js-yaml` `^4.3.1`→`^4.3.2` (GHSA-2883-xcg3-v3hh/CVE-2026-84375, `maxTotalMergeKeys` לא סופר merge sources ריקים אז DoS ב-CPU עדיין אפשרי — transitive דרך `eslint`'s `@eslint/eslintrc`, build/lint-time בלבד). אומת: `pnpm why sharp`/`pnpm why js-yaml` מראים את הגרסאות המתוקנות, `tsc`/`lint`/`test`/`build` עוברים.
