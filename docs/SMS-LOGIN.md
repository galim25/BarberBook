...
**סטטוס נכון ל-2026-09-22 (בוקר): הזרימה נבדקה מלאה בדפדפן במצב `mock` בשרת הפיתוח — כל התרחישים תקינים (ראו סעיף 4). לא נשלח אף SMS אמיתי מעולם. נשאר: לפתוח חשבון ב-019sms (פורמט ה-auth כבר אומת מול התיעוד הרשמי — נכון כפי שממומש), dry-run מול endpoint הבדיקות שלהם, ואז הדלקה אמיתית (סעיף 5).**
...
**לא נבדק:**
- שליחה אמיתית ב-SMS, שם שולח מאושר.
- `redirect()` אחרי כניסה מוצלחת ויצירת הסשן (הקוד עצמו זהה למה ש-`loginAction` עושה, אך לא הורץ).

**עודכן 2026-09-22 (בוקר) — בדיקה אינטראקטיבית בדפדפן בוצעה בשרת הפיתוח (מצב `mock`), כל התרחישים תקינים:**
לקוח קיים (כניסה ישירה), קוד שגוי (נדחה), שליחה חוזרת (קוד ישן מתבטל אוטומטית, רק החדש עובד), מספר חדש (שלב שם → יצירת חשבון → התראת `customer_registered` למנהל נוצרה ב-DB בפועל), מספר חסום (`/admin/blocked-customers` — נדחה גם אחרי שכבר נרשם), מספר מנהל (שלב "שליחה" "מצליח" בלי ליצור קוד אמיתי — כל קוד שמוזן נדחה), כניסת מנהל ב-`/login/admin` עם סיסמה. **להסיר `SMS_MOCK_REVEAL_CODE` ולשקול להחזיר `CUSTOMER_LOGIN_MODE` למצב הקודם עד שממשיכים לשלב 019 האמיתי.**

## 5. מה נשאר לעשות (לפי סדר)

1. **לפתוח חשבון ב-019sms:** נרשמים ב-`019sms.co.il` (מקבלים שם משתמש למייל), ואז בדשבורד: הגדרות ← ניהול Token API ← יצירת טוקן חדש (נשמר פעם אחת בלבד, לא ניתן לשחזור; עד 5 טוקנים פעילים). לברר איתם גם רישום/אישור שם שולח (`SMS_SENDER_ID`, עד 11 תווים) — לא מתועד, כנראה גם דרך הדשבורד. **פורמט ה-auth כבר אומת מול התיעוד הרשמי (`docs.019sms.co.il/guide/`) — `Authorization: Bearer <token>` נכון כפי שממומש, אין צורך לתקן קוד.**
2. **בדיקה במצב `mock` — בוצע 2026-09-22, ראו סעיף 4.**
3. **הרצת ניסיון מול ה-endpoint של הבדיקות של 019:** `SMS_PROVIDER="019"`, `SMS_019_USERNAME`, `SMS_019_TOKEN`, `SMS_SENDER_ID`, `SMS_019_ENDPOINT="https://019sms.co.il/api/test"`; לוודא שאין שגיאה ב-`pm2 logs` (`[sms-login] failed to send code`).
